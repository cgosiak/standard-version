...
function getMergePrefix () {
  var configPath = path.resolve(process.cwd(), 'package.json')
  var stat = fs.lstatSync(configPath)
  if (stat.isFile()) {
    var config = require(configPath)
    return (config && config.standardVersion && config.standardVersion.mergePrefix) || null
  } else {
    return null
  }
}
